...
void Matrix4x4_Multiply(float* dstMat, float* srcMatLeft, float* srcMatRight) {
    ...
    for (i = 0; i < MATRIX_4x4_ROW; ++i) {
            register const float srcMatRight_i0 = srcMatRight[INDEX(i, 0)];
            register float ri0 = srcMatLeft[INDEX(0,0)] * srcMatRight_i0;
            register float ri1 = srcMatLeft[INDEX(0,1)] * srcMatRight_i0;
            register float ri2 = srcMatLeft[INDEX(0,2)] * srcMatRight_i0;
            register float ri3 = srcMatLeft[INDEX(0,3)] * srcMatRight_i0;
            for (j = 1; j < MATRIX_4x4_COLUMN; j++) {
                    register const float srcMatRight_ij = srcMatRight[INDEX(i,j)];
